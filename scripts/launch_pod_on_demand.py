...
def normalise_gpu(name: str) -> str:
    """Ensure the string matches RunPod’s enum (must start with vendor)."""
    if name.startswith(("NVIDIA ", "AMD ", "Tesla ")):
        return name
...
def gather_prompts(pattern: str) -> str:
    """Concatenate all lines from every *.ndjson file that matches *pattern*."""
    lines: List[str] = []
...
def main() -> None:
    ...
    region = os.getenv("LINODE_DEFAULT_REGION") or sys.exit(
        "[ERROR] LINODE_DEFAULT_REGION missing."
    )
    ...
    # Qwen-3 params
    env_block = {
        "PROMPTS_NDJSON": gather_prompts(
            os.getenv("PROMPT_GLOB", "addendums/**/*.ndjson")
        ),
    ...
    payload = {
        ...
        "dockerStartCmd": [
            ...
            "-c",
            (
                # minimal OS + Python stack
                "export DEBIAN_FRONTEND=noninteractive && "
                "apt-get update -qq && "
                "apt-get install -y --no-install-recommends git python3-pip tzdata && "
                "python3 -m pip install --no-cache-dir --upgrade "
                "transformers>=4.51.0 accelerate bitsandbytes flash-attn pillow && "
                # pipeline repo
