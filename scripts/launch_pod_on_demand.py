...
• pip patch       → keep Torch 2.3.1 + cu118; no accidental upgrade
"""
...
def main() -> None:
    ...
    # ---------------------------------------------------------------------
    # START-UP COMMAND  (patched so torch *stays* at 2.3.1 + cu118)
    # ---------------------------------------------------------------------
    start_cmd = (
        ...
        # 1 ─ Install diffusers *without* pulling a new Torch, locked to cu118
        "python3 -m pip install --no-cache-dir --upgrade "
        "diffusers==0.33.1 "
        "transformers accelerate pillow safetensors xformers "
        "--extra-index-url https://download.pytorch.org/whl/cu118 && "

        # 2 ─ Re-affirm the baseline Torch & torchvision we want
        "python3 -m pip install --no-cache-dir --upgrade "
        "torch==2.3.1 torchvision==0.18.0+cu118 "
        "--extra-index-url https://download.pytorch.org/whl/cu118 && "

        # 3 ─ Pull / update the repo (cached between restarts)
        "[ -d /workspace/repo/.git ] || "
        "git clone --depth 1 https://github.com/CastlesideGameStudio/spec-render-pipeline.git /workspace/repo && "
        "cd /workspace/repo && "

        # 4 ─ Fire off the generator
    ...
    pod_json = resp.json()
    # API sometimes returns a list; normalise.
    ...
    # ── tail logs until the pod terminates ───────────────────────────────
    last_log = ""
    while True:
        time.sleep(POLL_SEC)
        ...
        status = requests.get(
            f"{API_PODS}/{pod_id}", headers=hdr, timeout=30
